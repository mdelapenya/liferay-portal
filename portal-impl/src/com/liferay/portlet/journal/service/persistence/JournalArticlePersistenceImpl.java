...
public class JournalArticlePersistenceImpl extends BasePersistenceImpl<JournalArticle>
	...
	public List<JournalArticle> findByUuid(String uuid, int start, int end,
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
			 ...
			 if (pagination) {
				query.append(JournalArticleModelImpl.ORDER_BY_ENTITY_ALIAS);
			}
	...
	private static final String _FINDER_COLUMN_UUID_UUID_3 = "(journalArticle.uuid_ IS NULL OR journalArticle.uuid = '')";
	...
	private static final String _FINDER_COLUMN_UUID_G_UUID_3 = "(journalArticle.uuid_ IS NULL OR journalArticle.uuid = '') AND ";
	...
	private static final String _FINDER_COLUMN_UUID_C_UUID_3 = "(journalArticle.uuid_ IS NULL OR journalArticle.uuid = '') AND ";
	...
	public List<JournalArticle> filterFindByGroupId(long groupId, int start,
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				query.append(JournalArticleModelImpl.ORDER_BY_ENTITY_TABLE);
			}
		}
	...
	public List<JournalArticle> findAll(int start, int end,
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				if (pagination) {
					sql = sql.concat(JournalArticleModelImpl.ORDER_BY_ENTITY_ALIAS);
				}
			}
	...
	public int countAll() throws SystemException {
		...
		if (count == null) {
			...
			try {
				...
				SQLQuery q = session.createSQLQuery(_SQL_COUNT_JOURNALARTICLE);
	...
	private static final String _SQL_COUNT_JOURNALARTICLE = "SELECT COUNT(*) AS COUNT_VALUE FROM JournalArticle journalArticle";
	private static final String _SQL_COUNT_JOURNALARTICLE_WHERE = "SELECT COUNT(*) AS COUNT_VALUE FROM JournalArticle journalArticle WHERE ";

...
public class JournalFeedPersistenceImpl extends BasePersistenceImpl<JournalFeed>
	...
	public List<JournalFeed> findByUuid(String uuid, int start, int end,
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
			 ...
			 if (pagination) {
				query.append(JournalFeedModelImpl.ORDER_BY_ENTITY_ALIAS);
			}
	...
	private static final String _FINDER_COLUMN_UUID_UUID_3 = "(journalFeed.uuid_ IS NULL OR journalFeed.uuid = '')";
	...
	private static final String _FINDER_COLUMN_UUID_G_UUID_3 = "(journalFeed.uuid_ IS NULL OR journalFeed.uuid = '') AND ";
	...
	private static final String _FINDER_COLUMN_UUID_C_UUID_3 = "(journalFeed.uuid_ IS NULL OR journalFeed.uuid = '') AND ";
	...
	public List<JournalFeed> filterFindByGroupId(long groupId, int start,
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				query.append(JournalFeedModelImpl.ORDER_BY_ENTITY_TABLE);
			}
		}
	...
	public List<JournalFeed> findAll(int start, int end,
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				if (pagination) {
					sql = sql.concat(JournalFeedModelImpl.ORDER_BY_ENTITY_ALIAS);
				}
			}
	...
	public int countAll() throws SystemException {
		...
		if (count == null) {
			...
			try {
				...
				SQLQuery q = session.createSQLQuery(_SQL_COUNT_JOURNALFEED);
	...
	private static final String _SQL_COUNT_JOURNALFEED = "SELECT COUNT(*) AS COUNT_VALUE FROM JournalFeed journalFeed";
	private static final String _SQL_COUNT_JOURNALFEED_WHERE = "SELECT COUNT(*) AS COUNT_VALUE FROM JournalFeed journalFeed WHERE ";

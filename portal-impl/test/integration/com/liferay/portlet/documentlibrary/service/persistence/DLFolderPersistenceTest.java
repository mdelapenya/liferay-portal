...
public class DLFolderPersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = dlFolder.getUuid();

		List<DLFolder> dlFolders = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, dlFolders.size());

		Assert.assertEquals(dlFolder.getPrimaryKey(),
			dlFolders.get(0).getPrimaryKey());
	}
	...
	public void testFindByUuidNotFound() throws Exception {
		...
		Assert.assertEquals(0, dlFolders.size());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		List<DLFolder> dlFolders = _persistence.findByUuid(uuid, start, end);

		Assert.assertEquals(expected, dlFolders.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		List<DLFolder> dlFolders = _persistence.findByUuid_C(uuid, companyId);

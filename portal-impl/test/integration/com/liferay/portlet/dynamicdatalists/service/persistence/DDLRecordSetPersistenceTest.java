...
public class DDLRecordSetPersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = ddlRecordSet.getUuid();

		List<DDLRecordSet> ddlRecordSets = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, ddlRecordSets.size());

		Assert.assertEquals(ddlRecordSet.getPrimaryKey(),
			ddlRecordSets.get(0).getPrimaryKey());
	}
	...
	public void testFindByUuidNotFound() throws Exception {
		...
		Assert.assertEquals(0, ddlRecordSets.size());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		List<DDLRecordSet> ddlRecordSets = _persistence.findByUuid(uuid, start,
				end);

		Assert.assertEquals(expected, ddlRecordSets.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		long companyId = ddlRecordSet.getCompanyId();

		List<DDLRecordSet> ddlRecordSets = _persistence.findByUuid_C(uuid,
				companyId);

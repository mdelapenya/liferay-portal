...
public class PollsQuestionPersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = pollsQuestion.getUuid();

		List<PollsQuestion> pollsQuestions = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, pollsQuestions.size());

		Assert.assertEquals(pollsQuestion.getPrimaryKey(),
			pollsQuestions.get(0).getPrimaryKey());
	}
	...
	public void testFindByUuidNotFound() throws Exception {
		...
		Assert.assertEquals(0, pollsQuestions.size());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		Assert.assertEquals(expected, pollsQuestions.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		long companyId = pollsQuestion.getCompanyId();

		List<PollsQuestion> pollsQuestions = _persistence.findByUuid_C(uuid,
				companyId);

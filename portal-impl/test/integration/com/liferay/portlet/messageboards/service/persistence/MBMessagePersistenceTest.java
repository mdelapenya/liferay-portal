...
public class MBMessagePersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = mbMessage.getUuid();

		List<MBMessage> mbMessages = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, mbMessages.size());

		Assert.assertEquals(mbMessage.getPrimaryKey(),
			mbMessages.get(0).getPrimaryKey());
	}
	...
	public void testFindByUuidNotFound() throws Exception {
		...
		Assert.assertEquals(0, mbMessages.size());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		List<MBMessage> mbMessages = _persistence.findByUuid(uuid, start, end);

		Assert.assertEquals(expected, mbMessages.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		List<MBMessage> mbMessages = _persistence.findByUuid_C(uuid, companyId);

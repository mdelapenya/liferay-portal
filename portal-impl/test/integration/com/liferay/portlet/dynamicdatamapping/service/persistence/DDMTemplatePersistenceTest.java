...
public class DDMTemplatePersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = ddmTemplate.getUuid();

		List<DDMTemplate> ddmTemplates = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, ddmTemplates.size());

		Assert.assertEquals(ddmTemplate.getPrimaryKey(),
			ddmTemplates.get(0).getPrimaryKey());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		List<DDMTemplate> ddmTemplates = _persistence.findByUuid(uuid, start,
				end);

		Assert.assertEquals(expected, ddmTemplates.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		List<DDMTemplate> ddmTemplates = _persistence.findByUuid_C(uuid,
				companyId);

...
public class WikiNodePersistenceTest {
	...
	public void testFindByUuid() throws Exception {
		...
		String uuid = wikiNode.getUuid();

		List<WikiNode> wikiNodes = _persistence.findByUuid(uuid);

		Assert.assertEquals(1, wikiNodes.size());

		Assert.assertEquals(wikiNode.getPrimaryKey(),
			wikiNodes.get(0).getPrimaryKey());
	}
	...
	public void testFindByUuidNotFound() throws Exception {
		...
		Assert.assertEquals(0, wikiNodes.size());
	}
	...
	protected void testFindByUuidStartEnd(int start, int end, int expected)
		...
		List<WikiNode> wikiNodes = _persistence.findByUuid(uuid, start, end);

		Assert.assertEquals(expected, wikiNodes.size());
	}
	...
	public void testFindByUuid_C() throws Exception {
		...
		List<WikiNode> wikiNodes = _persistence.findByUuid_C(uuid, companyId);

...
public final class ClassByteCodeMethodMatchers {
	...
	public static StringMatcherBuilder<ClassByteCodeMethod> methodName() {
		return new StringMatcherBuilder<ClassByteCodeMethod>(
			new RegexMatcherAdapter<ClassByteCodeMethod>() {

			@Override
			public String getStringToMatch(
				final ClassByteCodeMethod toBeAnalysed) {
	...
	public static FreudExtendedMatcher<ClassByteCodeMethod> hasMethodInvocation(
		final Class expectedOwner, final String expectedMethodName,
		final Class... expectedParamsDeclared) {
		...
		return new FreudExtendedMatcher<ClassByteCodeMethod>() {
...

			{
				...
				if (expectedParamsDeclared.length == 0) {
					expectedParamNames = EMPTY_ARGS;
				}
				else {
					expectedParamNames =
						new String[expectedParamsDeclared.length];
				}
			...
			protected boolean matchesSafely(final ClassByteCodeMethod item) {
				...
				item.findInstruction(new AbstractInstructionVisitor() {
					...
					public void methodInvocation(
						final Instruction instruction, final String owner,
			...
			public void describeTo(final Description description) {
				description.appendText(
					"hasMethodInvocation(" + expectedOwner.getName() + ", " +
					expectedMethodName + ", " +
					Arrays.toString(expectedParamNames) + ")");
			}

		};
	}

	public static FreudExtendedMatcher<ClassByteCodeMethod>
		...
		return new FreudExtendedMatcher<ClassByteCodeMethod>() {
					...
					public void methodInvocation(
						...
						if (!found[0] && expectedOwnerName.equals(owner) &&
							...
							Instruction prevInstruction =
								item.getInstruction(
									instruction.getInstructionIndex() - 1);

							OperandStack operandStack =
								prevInstruction.getOperandStack();

							found[0] = true;

							for (int i = expectedParamsPassed.length - 1; i >= 0; i--) {
								Matcher<OperandStack> matcher =
									expectedParamsPassed[i];
			...
			public void describeTo(final Description description) {
				description.appendText(
					"methodInvokedWithParams(" + expectedOwner.getName() +
					", " + expectedMethodName + ")");
			}
	...
	public static FreudExtendedMatcher<ClassByteCodeMethod>
		containsInstructions(final Opcode... opcodes) {
	...
	public static FreudExtendedMatcher<OperandStack> aConstantOf(
		final Class<?> aClass) {
	...
	public static FreudExtendedMatcher<OperandStack> castOf(
		final Class<?> aClass) {
			...
			protected boolean matchesSafely(final OperandStack item) {
				if (item instanceof CastOperandStack) {
					return expectedType.equals(
						((CastOperandStack)item).getFromType());
				}

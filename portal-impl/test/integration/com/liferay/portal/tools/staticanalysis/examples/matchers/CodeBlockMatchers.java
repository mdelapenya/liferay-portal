...
public final class CodeBlockMatchers {
	...
	public static FreudExtendedMatcher<CodeBlock> hasMethodCall(
		...
		return new FreudExtendedMatcher<CodeBlock>() {
...

			{
				...
				System.arraycopy(
					values, 0, instanceReferences, 0, instanceReferences.length);
			...
			protected boolean matchesSafely(final CodeBlock toBeAnalysed) {
				List<MethodCall> methodCallList =
					toBeAnalysed.getMethodCallListByMethodName(methodName);

				if (methodCallList != null) {
					for (MethodCall methodCall : methodCallList) {
						if (Arrays.equals(
								instanceReferences,
								methodCall.getInstanceReferences())) {
	...
	public static IntOperatorMatcherBuilder<CodeBlock> codeBlockLines() {
		return new IntOperatorMatcherBuilder<CodeBlock>(
			new IntOperatorMatcherAdapter<CodeBlock>() {
	...
	public static FreudExtendedMatcher<CodeBlock> method(
		final FreudExtendedMatcher<MethodDeclaration> methodMatcher) {
